...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Body scroll locking - handled via JS body-scroll-lock pattern */
...
<body>
  ...
  <script>
    ...
    function _onOverlayTouchMove(e) {
      const scrollEl = document.getElementById('leadModalScrollContent');
      if (!scrollEl) { e.preventDefault(); return; }

      /* If touch is NOT inside the scroll container → block completely */
      if (!scrollEl.contains(e.target)) {
        e.preventDefault();
        return;
      }

      /* Touch IS inside scroll container – only block at edges */
      const deltaY = _modalTouchStartY - e.touches[0].clientY;
    ...
    function openLeadModal(e) {
      ...
      /* Prevent overlay touch-scroll leaking to body */
      modal.addEventListener('touchstart', _onOverlayTouchStart, { passive: true });
      modal.addEventListener('touchmove',  _onOverlayTouchMove,  { passive: false });
    ...
    function closeLeadModal() {
      ...
      modal.removeEventListener('touchstart', _onOverlayTouchStart);
      modal.removeEventListener('touchmove',  _onOverlayTouchMove);

      /* Body-scroll-unlock: restore position */
